...
{	 
    ...
    /**
     * Dispatched on when new user account registration removal is successful.
     * 
     * @eventType org.jivesoftware.xiff.events.RemoveRegistrationSuccessEvent.REMOVE_REGISTRATION_SUCCESS
     */
    [Event(name="removeRegistrationSuccess", type="org.jivesoftware.xiff.events.RemoveRegistrationSuccessEvent")]
	...
	public class XMPPConnection extends EventDispatcher
	{
		...
		{
			var regIQ:IQ = new IQ( new JID(server), IQ.SET_TYPE, XMPPStanza.generateID("unreg_attempt_"), "removeRegistration_result", this, null );
			var ext:RegisterExtension = new RegisterExtension(regIQ.getNode());
			ext.unregister = true;
			send( regIQ );
		}
		 ...
		 * @private
		 */
		protected function removeRegistration_result( resultIQ:IQ ):void
		{
			if( resultIQ.type == IQ.RESULT_TYPE ) {

				var event:RemoveRegistrationSuccessEvent = new RemoveRegistrationSuccessEvent();
				dispatchEvent( event );
			}
			else {
				// We weren't expecting this
